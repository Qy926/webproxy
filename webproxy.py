...
def get_policy_info():
    ...
    extra = {'trace_id': trace_id, 'reponame': 'webproxy'}
    ...
    if not user_name or "@" not in user_name:
        logging.error("Invalid username:%s", user_name, extra=extra)
...
def get_user_traffic_usage():
    ...
    logging.info("User traffic usage info calculated, username:%s, traffic info:%r", user_name, traffic_info, extra=extra)
...
def get_group_traffic_usage():
    ...
    if not company_id or not group_name:
        logging.error("Invalid company id or group name, company id:%s, group name:%s", company_id, group_name, extra=extra)
        return jsonify({"error": "Invalid company id or group name"}), 400
    ...
    logging.info("Group traffic usage info calculated, company id:%s, group name:%s, traffic info:%r", company_id, group_name, traffic_info, extra=extra)
    return jsonify({"traffic": traffic_info}), 200
...
def get_user_billing_usage():
    ...
    if company_billing_info is None:
        return jsonify({"error": "No company billing info found"}), 404
    ...
    if user_billing_info is None:
        return jsonify({"error": "No user billing info found"}), 404
    ...
    logging.info("User billing usage info calculated, username:%s, billing info:%r", user_name, billing_info, extra=extra)
...
def get_group_billing_usage():
    ...
    logging.info("Group billing usage info calculated, company id:%s, group name:%s, billing info:%r", company_id, group_name, billing_info, extra=extra)
    return jsonify({"billing": billing_info}), 200
